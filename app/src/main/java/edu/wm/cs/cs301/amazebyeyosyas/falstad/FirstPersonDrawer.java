...
import edu.wm.cs.cs301.amazebyeyosyas.generation.BSPBranch;
import edu.wm.cs.cs301.amazebyeyosyas.generation.BSPLeaf;
...
import edu.wm.cs.cs301.amazebyeyosyas.generation.MazeBuilder;
import edu.wm.cs.cs301.amazebyeyosyas.generation.Seg;
import edu.wm.cs.cs301.amazebyeyosyas.generation.Wall;
...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 */
	@Override
	public void redraw(MazePanel panel /*Graphics gc*/, StateGUI state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		*/
		...
		// TODO: (Not sure if setRenderingHints() is needed anymore) this.panel.setRenderingHints();
		...
		drawBackground(this.panel); 	//drawBackground(gc);
		
		// set color to white and draw what ever can be seen from the current position
		this.panel.setColor(Colors.WHITE);
	...
	private void drawBackground(MazePanel panel) {
        ...
        panel.drawMazeBackground(0, 0, view_width, view_height);
	}
	...
	private void drawSegment(Seg seg) {
		...
		int x1i = x1;
		int xd = x2-x1;
		panel.setSegColor(seg.getColor());
		boolean drawn = false;
		...
		while (x1i <= x2) {
			...
			*/
			/*panel.setSegColor(new int[]{240,20,20});
			panel.getGraphicsObj().drawPolygon(xps,yps,4);
			panel.setSegColor(seg.getColor());*/
			// end debugging
			drawn = true;
			rset.remove(x1i, x2i);
