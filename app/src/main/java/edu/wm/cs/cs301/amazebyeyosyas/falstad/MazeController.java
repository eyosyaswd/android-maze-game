...
import edu.wm.cs.cs301.amazebyeyosyas.falstad.Constants.StateGUI;
import edu.wm.cs.cs301.amazebyeyosyas.generation.CardinalDirection;
import edu.wm.cs.cs301.amazebyeyosyas.generation.Cells;
import edu.wm.cs.cs301.amazebyeyosyas.generation.Factory;
...
public class MazeController implements Order {
	...
	public MazeController() {
		...
		// TODO: (initialize panel correctly) panel = new MazePanel() ;
	...
	public MazeController(Order.Builder builder)
	{
		super() ;
		setBuilder(builder) ;
        // TODO: initialize panel correctly
        // panel = new MazePanel() ;
		mazeConfig = new MazeContainer();
		factory = new MazeFactory() ;
		filename = null;
	...
	/**
	 * Constructor that selects a particular generation method, a skill level, and perfection of maze.
	 * @param builder
	 * @param skillLevel
	 * @param perfect
	 */
	public MazeController(Order.Builder builder, int skillLevel, boolean perfect) {
		super() ;
		setBuilder(builder); 
	...
	public void init() {
		...
		if (null != filename) {
			...
			rset = new RangeSet();
			//TODO: (initBufferImage not needed anymore?)panel.initBufferImage() ;
			//TODO: (MazeView not used anymore) addView(new MazeView(this)) ;
	...
	/**
	 * Switches to title screen.
	 * This transition is only possible from the playing screen when
	 * the robot runs out of battery.
	 */
	protected void switchToLostScreen() {
		assert state == StateGUI.STATE_PLAY : "MazeController.switchToLostScreen: unexpected current state " + state ;
		setState(StateGUI.STATE_LOST);
		notifyViewerRedraw();
	}
	/**
	 * Switches to title screen, possibly canceling maze generation.
	...
	synchronized private void rotate(int dir) {
		//System.out.println("Before rotation: " + getCurrentDirection()); 	// prints the direction before the rotation
		...
		//System.out.println("After the rotation: " + getCurrentDirection()); 	// prints the direction after the rotation
	}
	/**
	...
	protected void setState(StateGUI newState) {
		...
		case STATE_GENERATING:
			...
			break;
		case STATE_PLAY:
			assert (newState == StateGUI.STATE_TITLE || newState == StateGUI.STATE_FINISH || newState == StateGUI.STATE_LOST) : msg ;
			break;
		case STATE_LOST:
	...
	public void deliver(MazeConfiguration mazeConfig) {
		...
		// drives robot to exit if there is a robot and a robot driver
		if (robotDriver != null && robot != null) {
			robotDriver.setDimensions(this.mazeConfig.getWidth(), this.mazeConfig.getHeight());
			robotDriver.setDistance(this.mazeConfig.getMazedists());
	...
	public void updateProgress(int percentage) {
        //Log.v("MazeController", "MazeController.updateProgress(): " + percentdone);
	...
	/**
	 * Gets the maze cells from the maze configuration
	 * @return
	 */
